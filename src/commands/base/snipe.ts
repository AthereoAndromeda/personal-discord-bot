...
import { isIndexOverMaxSnipes, main } from "../helpers/snipe";

export default <Command>{
  data: {
    name: "snipe",
    description: "Fetches sniped and deleted messages",
    options: [
      {
        name: "ephemeral",
  ...
  async executeMessage(message, args) {
    try {
      ...
      const embed = main(message.client, message.channelId, index, "snipe");
  ...
  async executeInteraction(interaction) {
    try {
      ...
      const embed = main(interaction.client, channelId, index, "snipe");
      interaction.reply({
        embeds: [embed],
      ...
      interaction.reply(`Error Sending! \`${error}\``);
    }
  },
};
